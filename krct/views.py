...
def createpost(request):
        if request.method == 'POST':
            if request.POST.get('title') and request.POST.get('date'):
                ...
                CtPost.objects.create(title=title,date=date,time=time,img=img)
...
def home(request):
    ...
    post = CtPost.objects.all()
    if request.method == "POST":
        item = request.POST.get('search')
        print(item)
        post = CtPost.objects.filter(title__contains=item) 
        context = {'posts':post}
